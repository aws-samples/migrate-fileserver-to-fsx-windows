...
$isShared = Get-SmbShare -Path $ShareRootFolder -ErrorAction SilentlyContinue
if ($isShared) {
    ...
    Write-Host "Copying shared top-level folder: $ShareRootFolder" -ForegroundColor Green
    robocopy $ShareRootFolder $FSxDriveLetter /copy:DATSOU /secfix /e /b /MT:32 /XD '$RECYCLE.BIN' "System Volume Information" /V /TEE /LOG+:"$logFilePath"
}else{
